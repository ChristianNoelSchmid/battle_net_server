...
use self::{error::{Result, AuthServiceError}, data_layer::{AuthDataLayer, entities::RefrTokenDbModel}};
...
pub trait AuthService: Send + Sync {
    ...
    async fn create_new_user(&self, email: String, pwd: String, card_idx: i64) -> Result<()>;
}
    ...
    async fn try_accept_creds(&self, email: String, pwd: String) -> Result<TokensDto> {
        ...
        if let Some(user) = user {
            ...
            if matches {
                ...
                self.data_layer.create_refr_token(user.id, tokens.refr_token.value())
    ...
    async fn try_accept_refresh(&self, token: String) -> Result<TokensDto> {
        ...
        if let Some(refr_token) = refr_token {
            ...
            if let Some(user) = user {
                ...
                let repl_id = self.data_layer.create_refr_token(user.id, tokens.refr_token.value())
...
async fn revoke_token(refr_token: RefrTokenDbModel, data_layer: &Arc<dyn AuthDataLayer>) -> data_layer_error::Result<i64> {
